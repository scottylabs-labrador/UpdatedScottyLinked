...
export default function Network({ profiles, loading, connectedIds = [], onConnectionCreated }: NetworkProps) {
  const [connectingUsers, setConnectingUsers] = useState<Set<number>>(new Set());
  // Split profiles
  const connectedProfiles = profiles.filter(profile => connectedIds.includes(profile.id));
  const notConnectedProfiles = profiles.filter(profile => !connectedIds.includes(profile.id));
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {connectedProfiles.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-6 text-center text-gray-500 mb-6">No connections yet.</div>
      ) : (
            ...
            <div key={profile.id} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {profile.skills.slice(0, 3).map((skill: string, idx: number) => (
                  <span key={idx} className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs font-medium">{skill}</span>
                ))}
              </div>
      ...
      {/* New People Section */}
      <h2 className="text-lg font-semibold mb-2">Discover New People</h2>
      {notConnectedProfiles.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-6 text-center text-gray-500">No new people to discover right now.</div>
      ) : (
