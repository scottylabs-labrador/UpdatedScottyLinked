...
import { rejectConnection } from "@/lib/db/connections";
...
/** POST: reject a connection request (caller must be the receiver). */
export async function POST(
  ...
  const supabase = await createClient();
  const ok = await rejectConnection(supabase, connectionId, currentId);
  if (!ok) {
